...
// Creates a random matrix given the dimensions and performs a straight-forward
// convolution on a random square mask given the dimension without fancy
// optimizations.
//
// Params:
...
void conv(size_t inputHeight, size_t inputWidth, size_t maskDimension,
              size_t partitions) {
...
// A simple program that performs a simple convolution using the libeventdisp
// library.
//
// Params:
...
int main(int argc, char **argv) {
  if (argc < 4 || argc > 5) {
    cerr << "usage: " << argv[0]
         << " <partitions> <partitionHeight> <width> [<threads>]" << endl;
    return -1;
  }
  ...
  conv(height, width, MASK_DIMENSION, partitions);
